"""Generate cost-aware boosting single-asset report."""
...
def main() -> None:
    ...
    result = run_cost_aware_boosting_experiment(prices, config=config, data_snapshot=snapshot)
    ...
    payload["notes"] = [
        "Cost-aware labels drop neutral future returns inside the +/- buffer.",
        "HistGradientBoostingClassifier is used instead of RandomForest for this experiment.",
        "Probability threshold is selected on validation data inside the train period.",
    ]
    metrics_path.write_text(json.dumps(_json_safe(payload), indent=2, sort_keys=True) + "\n")
    write_cost_aware_figure(result.equity_curves, result.drawdowns, figure_path)
...
def parse_args() -> argparse.Namespace:
    default_metrics = reports_dir() / "metrics" / "single_asset_cost_aware_boosting.json"
    default_figure = reports_dir() / "figures" / "single_asset_cost_aware_boosting.png"
    ...
    parser.add_argument("--cost-buffer", type=float, default=0.0010)
...
def write_cost_aware_figure(equity_curves, drawdowns, output_path: Path) -> None:
    fig, axes = plt.subplots(2, 1, figsize=(12, 8), sharex=True, constrained_layout=True)
    equity_curves.plot(ax=axes[0], linewidth=1.2)
    axes[0].set_title("Cost-aware BTCUSDT boosting experiment")
